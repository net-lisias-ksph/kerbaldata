            ...
            for (var i = 0; i < data.Count; i++)
            {
                ...
                else if (currentLine.Contains("=")) // If the line is a value assignment (in ksp files "=" is only value assignment for fields. Child object names act as the field name for complex objects. 
                {
                    ...
                    else
                    {
                        ...
                        //rightSide = rightSide != string.Empty ? JsonEncode(rightSide.Trim()) : "\"\"";

                        if (!leftSide.ToUpper().Equals("CDATA"))
                        {
                            rightSide = JsonEncode(rightSide.Trim());
                        }
                        else
                        {
                            rightSide = HttpServerUtility.UrlTokenEncode(Encoding.UTF8.GetBytes(rightSide));
                        }
                    }
